<template>
  <div>
    ...
    >
      ...
      <LControl
        v-if="infoTextPercent && infoTextName && infoTextNumber"
        position="topright"
      >
        <div class="legend-box">
...
const resultkeys = ["Pdl", "CDU", "SPD", "B90", "AfD", "BSW", "FDP"];
...
const geoStyler = (feature: any) => {
  ...
  if (mode.value === "Die Linke") {
    fillColor = getFillColor(
      "Pdl",
      feature.properties["votes-in-percent-resultPdl"],
    );
  } else if (mode.value === "Gewinner") {
    fillColor = getFillColor(
      getWinner(feature),
      feature.properties[`votes-in-percent-result${getWinner(feature)}`],
    );
  }
  return {
    fillOpacity: 0.8,
...
const getWinner = (feature: any) => {
  ...
  resultkeys.forEach((key) => {
    ...
    if (
      Number(feature.properties[`votes-in-percent-result${key}`]) >
      Number(winnerResult)
    ) {
      console.log(winnerResult);
      winnerResult = Number(
        feature.properties[`votes-in-percent-result${key}`],
      );
      winnerParty = key;
    }
  });
...
function highlightFeature(e: any) {
  ...
  if (mode.value === "Die Linke") {
    infoTextPercent.value = potential.value
      ? layer.feature.properties["potential"].toFixed(2)
      : layer.feature.properties["votes-in-percent-resultPdl"];
    infoTextParty.value = "PdL";
  } else if (mode.value === "Gewinner") {
    infoTextPercent.value =
      layer.feature.properties[
        `votes-in-percent-result${getWinner(layer.feature)}`
      ];
...
const loadGeoJson = async (vote: string, area: string, potential = false) => {
  ...
  if (geojson.value) {
    if (potential) {
      const dataset = geojson.value.features.map(
        (feature: any) => feature.properties["participation"] * 100,
      );
      ...
      geojson.value.features.forEach((feature: any) => {
        const newOpacity =
          Math.round(
            ((feature.properties["participation"] * coefficient) / 100) * 10,
          ) / 10;
      ...
      if (mode.value === "Die Linke") {
        const dataset = geojson.value.features.map(
          (feature: any) => feature.properties["votes-in-percent-resultPdl"],
        );
        infoLegendMax.value = getMax(dataset).toFixed(2);
        infoLegendMin.value = getMin(dataset).toFixed(2);
      } else if (mode.value === "Gewinner") {
