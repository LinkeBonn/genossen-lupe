  ...
  <div class="content-box">
    <div class="content">
      ...
      <div v-if="areaDatasource" class="detailed-box">
        <div
          v-for="(areapoint, index) in areaDatasource"
          :key="index"
          class="detail"
        >
...
const generatedAreaDatasource = (area: string) => {
  ...
  if (datasource.value) {
    if (area === "Bonn") {
      ...
      const areaFound = datasource.value[0].results.find(
        (probe: any) => probe.name === area,
      );
          ...
          element.results.forEach((result: any) => {
            result.areas.forEach((resultArea: any) => {
              if (resultArea.area === areaNumber) {
                areaResults.push({
                  result: resultArea.result,
                  name: element.poll,
                  areaname: resultArea.name,
                });
              }
            });
          });
